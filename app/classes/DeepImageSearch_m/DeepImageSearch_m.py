import app.classes.DeepImageSearch_m.config as config
import os
import pandas as pd
...
class Search_Setup:
    ...
    def __init__(self, image_list: list, model_name='vgg19', pretrained=True, image_count: int = None, cat = 'models/Магнитные трековые системы'):
        """
        Parameters:
        -----------
        image_list : list
        ...
        self.d = None
        self.cat=translit(cat, 'ru', reversed=True)
        #self.cat = cat
        ...
        if f'{self.cat}/metadata-files/{self.model_name}' not in os.listdir():
            try:
                os.makedirs(f'{self.cat}/metadata-files/{self.model_name}')
    ...
    def _start_feature_extraction(self):
        ...
        image_data.to_pickle(config.image_data_with_features_pkl(self.model_name, self.cat))
    ...
    def _start_indexing(self, image_data):
        ...
        pathka= config.image_features_vectors_idx(self.model_name, self.cat)
        # with open(pathka, "w") as file:
        #     file.write("Hello, world!")

        # pathka = f'models/metadata-files/vgg19/image_features_vectors.idx'
        faiss.write_index(index, pathka)

        #faiss.write_index(index, 'image_features_vectors.idx')
        print("\033[94m Saved The Indexed File:" + f"[metadata-files/{self.model_name}/image_features_vectors.idx]")

    def run_index(self, cat='default'):
        """
        Indexes the images in the image_list and creates an index file for fast similarity search.
        """
        if len(os.listdir(f'{self.cat}/metadata-files/{self.model_name}')) == 0:
            ...
            if flag.lower() == 'yes':
                ...
                print(os.listdir(f'{self.cat}/metadata-files/{self.model_name}'))
        self.image_data = pd.read_pickle(config.image_data_with_features_pkl(self.model_name, self.cat))
    ...
    def add_images_to_index(self, new_image_paths: list):
        ...
        self.image_data = pd.read_pickle(config.image_data_with_features_pkl(self.model_name, self.cat))
        index = faiss.read_index(config.image_features_vectors_idx(self.model_name, self.cat))
        ...
        self.image_data.to_pickle(config.image_data_with_features_pkl(self.model_name, self.cat))
        faiss.write_index(index, config.image_features_vectors_idx(self.model_name, self.cat))
